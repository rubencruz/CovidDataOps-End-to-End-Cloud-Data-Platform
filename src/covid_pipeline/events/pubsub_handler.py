...
def _decode_pubsub_data(event: dict[str, Any]) -> dict[str, Any]:
    """Decode the Pub/Sub message data delivered by Eventarc."""
    ...
    if not isinstance(payload, dict):
        raise ValueError("Decoded Cloud Storage notification must be a JSON object")
    return payload


def extract_gcs_object(event: dict[str, Any]) -> tuple[str, str]:
    """Return (bucket, object_name) from a Pub/Sub/Eventarc event."""
    payload = _decode_pubsub_data(event)
    bucket = payload.get("bucket") or BUCKET_NAME
    object_name = payload.get("name")

    if not object_name:
        raise ValueError("Cloud Storage notification is missing object name")
    if not bucket:
        raise ValueError("Cloud Storage notification is missing bucket name")
